...
class Mailer < ActionMailer::Base
  ...
  def reminder(user, issues, days, days_start)
    @issues = issues
    @days = days
    ...
    if days_start != 0
      ...
      @open_issues_url = url_for(:controller => 'issues', :action => 'index',
                                  ...
                                  :sort => 'start_date:asc')
      @reminder_issues_url = url_for(:controller => 'issues', :action => 'index',
        :set_filter => 1, :sort => 'start_date:asc',
        :f => ['status_id', 'assigned_to_id', "start_date"],
        :op => {'status_id' => 'o', 'assigned_to_id' => '=', 'start_date' => '<t+'},
        :v =>{'assigned_to_id' => ['me'], 'start_date' => [days_start]})
    end
  ...
  def self.reminders(options={})
    days = options[:days] || 7
    days_start = options[:days_start] || 0
      ...
      scope =
        Issue.open.where(
          "#{Issue.table_name}.assigned_to_id IS NOT NULL" \
            ...
            " OR #{Issue.table_name}.due_date <= ?)", days_start.day.from_now.to_date, days.day.from_now.to_date
        )
    else
      scope =
        Issue.open.where(
    ...
    issues_by_assignee.each do |assignee, issues|
      if assignee.is_a?(User) && assignee.active? && issues.present?
        ...
        if days_start != 0
          visible_issues.sort!{|a, b| (a.start_date <=> b.start_date).nonzero? || (a.id <=> b.id)}
        else
          visible_issues.sort!{|a, b| (a.due_date <=> b.due_date).nonzero? || (a.id <=> b.id)}
        end
        reminder(assignee, visible_issues, days, days_start).deliver_later if visible_issues.present?
      end
    end
  end
